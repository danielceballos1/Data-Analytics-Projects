...
-- view top violation descriptions 

SELECT violation_description, COUNT(violation_description) AS num_violations
FROM sf_restaurant_scores
...
-- find businesses with issues related to the water
...
-- find all businesses whose lowest inspection score is 100

SELECT business_name, MIN(inspection_score)
FROM sf_restaurant_scores
GROUP BY business_name
HAVING MIN(inspection_score) = 100
ORDER BY business_name ASC

-- find all businesses whose lowest and highest inspection score are different

SELECT business_name, MIN(inspection_score), MAX(inspection_score)
FROM sf_restaurant_scores
GROUP BY business_name
HAVING MIN(inspection_score) != MAX(inspection_score)
ORDER BY business_name ASC

-- find businesses with lowest inspection score 
